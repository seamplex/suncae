...
for ($i = 0; $i < 10; $i++) {
  ...
  $bc_type = "custom";
  $bc_active = ($i < count($bc));
  $custom_value = ($bc_active) ? $bc[$i]["value"] : $default_bc[$problem];

  $u = "";
  $v = "";
  $w = "";
  if ($bc_active && str_contains($bc[$i]["value"], "u=")) {
    preg_match('/u=([^\s]*)/', $bc[$i]["value"], $matches);
    if (count($matches) > 1) {
      $u = $matches[1];
      $bc_type = "displacement";
    }
  }
  if ($bc_active && str_contains($bc[$i]["value"], "v=")) {
    preg_match('/v=([^\s]*)/', $bc[$i]["value"], $matches);
    if (count($matches) > 1) {
      $v = $matches[1];
      $bc_type = "displacement";
    }
  }
  if ($bc_active && str_contains($bc[$i]["value"], "w=")) {
    preg_match('/w=([^\s]*)/', $bc[$i]["value"], $matches);
    if (count($matches) > 1) {
      $w = $matches[1];
      $bc_type = "displacement";
    }
  ...
  $u_fixed = $bc_active && str_contains($bc[$i]["value"], "u=0");
  $v_fixed = $bc_active && str_contains($bc[$i]["value"], "v=0");
  $w_fixed = $bc_active && str_contains($bc[$i]["value"], "w=0");
  if ($u_fixed || $v_fixed || $w_fixed) {
    $bc_type = "fixture";
  }

  $p = "0";
  if ($bc_active && str_contains($bc[$i]["value"], "p=")) {
  ...
  $Fy = "0";
  $Fz = "0";
  if ($bc_active && str_contains($bc[$i]["value"], "Fx=")) {
    preg_match('/Fx=([^\s]*)/', $bc[$i]["value"], $matches);
    $Fx = $matches[1];
    $bc_type = "force";
  }
  if ($bc_active && str_contains($bc[$i]["value"], "Fy=")) {
    preg_match('/Fy=([^\s]*)/', $bc[$i]["value"], $matches);
    $Fy = $matches[1];
    $bc_type = "force";
  }
  if ($bc_active && str_contains($bc[$i]["value"], "Fz=")) {
...
?>
 <div class="accordion-item <?=($bc_active) ? "d-block" : "d-none" ?>" id="div_bc_<?=$i+1?>">
    ...
    <div class="row mb-1">
     <div class="col-4">
      <select class="form-select" id="bc_what_<?=$i+1?>" onchange="bc_change_filter(<?=$i+1?>, this.value)">
       <option value="2"<?=($bc_active && $bc[$i]["entities"] == "face") ? " selected" : ""?>>Faces</option>
       <option value="1"<?=($bc_active && $bc[$i]["entities"] == "edge") ? " selected" : ""?>>Edges</option>
       <option value="0"<?=($bc_active && $bc[$i]["entities"] == "point") ? " selected" : ""?>>Vertices</option>
      </select>
     </div> 

     <div class="col-8">
      <div class="input-group">
       <input type="text" class="form-control" name="bc_<?=$i+1?>_groups" id="text_bc_<?=$i+1?>_groups" value="<?=($bc_active) ? $bc[$i]["groups"] : ""?>" onblur="bc_update_from_text(<?=$i+1?>)" disabled>
    ...
    <div class="row mb-1">
     <div class="col-4">
      <select class="form-select" id="bc_what_<?=$i+1?>" onchange="bc_update_type(<?=$i+1?>, this.value)">
       ...
       <option value="displacement" <?=($bc_type == "displacement")?"selected":""?>>Displacement</option>
     ...
     <!-- displacement -->
     <div class="col-8 <?=($bc_type == "displacement")?"":"d-none"?>" id="bc_value_<?=$i+1?>_displacement">
      <div class="row">
